...
class DatabaseSeeder extends Seeder
{
    public function run(): void
    {
        ...
        foreach ([112, 118, 131, 149, 166, 178, 154, 137, 121, 116, 109, 114] as $minutes => $value) {
            GlucoseReading::query()->firstOrCreate(
                ['patient_id' => $patient->id, 'timestamp' => now()->subMinutes((12 - $minutes) * 10)],
                [
                    'device_id' => $device?->id,
                    'value' => $value,
                    'unit' => 'mg/dL',
                    'trend' => $value > 160 ? 'rising' : 'steady',
                    'status' => GlucoseReading::statusForValue($value),
                    'source' => 'seed',
                ],
            );
        }
